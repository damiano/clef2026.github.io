---
title: "Program"
draft: false
summary: "The program of CLEF 2026."
weight: 60
toc: true
menu:
  main:
    identifier: "conference-program"
    parent: "conference"
    weight: 60
---

The CLEF 2026 conference will be held from September 21-24, 2026, at [Friedrich-Schiller-Universität Jena, Germany](/conference/venue). The program includes keynote speeches, paper presentations, lab sessions, poster presentations, and social events.

## Monday, 21 September 2026

{{< rawhtml >}}
<style>
    table {
        border: gray 1px solid;
        background: transparent;
    }
    th {
        text-align: center;
        border-bottom: gray 1px solid;
        background: rgb(0 89 138);
        color: white;
    }
    td {
        padding: 4px;
        text-align: left;
        border-bottom: gray 1px solid;
    }
    .room {
        color: gray;
    }
    .event {
        border: gray 1px solid;
    }

    .main-conf {
        background: rgb(254, 230, 133, 0.3);
    }
    .break {
        background: rgb(231, 229, 228);
    }
    .labs {
        background: rgb(254, 230, 133, 0.6);
    }
    .social {
        background: rgb(184, 230, 254, 1.0);
        
        /*color: black;*/
    }
</style>
{{< /rawhtml >}}


[//]: # (======================================================================================================================)
[//]: # (Monday)
{{< rawhtml >}}
<table>
<colgroup>
    <col span="3" class="time">
    <col span="6" class="event">
</colgroup>
<thead>
    <tr>
        <th colspan="9">Monday, 21 September 2026</th>
    </tr>
</thead>
<tbody>
<tr class="social">
    <td>08:15</td>
    <td>&ndash;</td>
    <td>09:00</td>
    
    <td colspan="6">Registration</td>
</tr>
<tr class="main-conf">
    <td>09:00</td>
    <td>&ndash;</td>
    <td>09:30</td>
    <td colspan="6">Opening<br><span class="room">Lecture Room 3</span></td>
</tr>
<tr class="main-conf">
    <td>09:30</td>
    <td>&ndash;</td>
    <td>10:30</td>
    <td colspan="6">Keynote: Michael Granitzer<br><span class="room">Lecture Room 3</span></td>
</tr>
<tr class="break">
    <td>10:30</td>
    <td>&ndash;</td>
    <td>11:00</td>
    <td colspan="6">Coffee Break &amp; Poster Session (Short Paper, LifeCLEF, EXIST, Touché, FinMMEval)</td>
</tr>
<tr class="main-conf">
    <td>11:00</td>
    <td>&ndash;</td>
    <td>11:30</td>
    <td colspan="6">Main Conference 1 (Low-Resource Languages)<br><span class="room">Lecture Room 3</span></td>
</tr>
<tr class="labs">
    <td>11:30</td>
    <td>&ndash;</td>
    <td>12:30</td>
    <td colspan="6">Lab Overviews 1 (SimpleText, LifeCLEF, EXIST, qCLEF, Touché, FinMMEval)<br><span class="room">Lecture Room 3</span></td>
</tr>
<tr class="break">
    <td>12:30</td>
    <td>&ndash;</td>
    <td>14:00</td>
    <td colspan="6">Lunch Break</td>
</tr>
<tr class="labs">
    <td>14:00</td>
    <td>&ndash;</td>
    <td>15:30</td>
    <td>SimpleText<br>1/3</td>
    <td>LifeCLEF<br>1/3</td>
    <td>EXIST<br>1/2</td>
    <td>qCLEF<br>1/2</td>
    <td>Touché<br>1/4</td>
    <td>FinMMEval<br>1/2</td>
</tr>
<tr class="break">
    <td>15:30</td>
    <td>&ndash;</td>
    <td>16:00</td>
    <td colspan="6">Coffee Break &amp; Poster Session (Short Paper, LifeCLEF, EXIST, Touché, FinMMEval)</td>
</tr>
<tr class="labs">
    <td>16:00</td>
    <td>&ndash;</td>
    <td>17:30</td>
    <td>SimpleText<br>2/3</td>
    <td>LifeCLEF<br>2/3</td>
    <td>EXIST<br>2/2</td>
    <td>qCLEF<br>2/2</td>
    <td>Touché<br>2/4</td>
    <td>FinMMEval<br>2/2</td>
</tr>
<tr class="social">
    <td>18:30</td>
    <td></td>
    <td></td>
    <td colspan="6">Welcome Reception</td>
</tr>
</tbody>
</table>
{{< /rawhtml >}}

**Papers in Main Conference 1 (Low-Resource Languages)**
1. *Model Comparison for Kazakh Semantic Reuse Detection Across Pairwise, News, and Long-Document Evaluation*<br>
  Bakhyt Bakiyev, Shuo Wang and Mubashir Ali
2. *Privacy-Preserving NLP for a Low-Resource Language: Benchmarking LLM-Based PII Anonymization in Slovene*<br>
  Nenad Jakovchevski and Matej Martinc

[//]: # (======================================================================================================================)
[//]: # (Tuesday)

## Tuesday, 22 September 2026

{{< rawhtml >}}
<table>
<colgroup>
    <col span="3" class="time">
    <col span="6" class="event">
</colgroup>
<thead>
    <tr>
        <th colspan="9">Tuesday, 22 September 2026</th>
    </tr>
</thead>
<tbody>
<tr class="main-conf">
    <td>09:00</td>
    <td>&ndash;</td>
    <td>10:00</td>
    <td colspan="6">Keynote: Lucie Flek<br><span class="room">Lecture Room 3</span></td>
</tr>
<tr class="break">
    <td>10:00</td>
    <td>&ndash;</td>
    <td>10:30</td>
    <td colspan="6">Coffee Break &amp; Poster Session (HIPE, BioASQ, ELOQUENT)</td>
</tr>
<tr class="main-conf">
    <td>10:30</td>
    <td>&ndash;</td>
    <td>11:15</td>
    <td colspan="6">Main Conference 2 (Bias, Fairness, Evidence)<br><span class="room">Lecture Room 3</span></td>
</tr>
<tr class="labs">
    <td>11:15</td>
    <td>&ndash;</td>
    <td>12:05</td>
    <td colspan="6">Lab Overviews 2 (ImageCLEF, HIPE, TalentCLEF, BioASQ, ELOQUENT)<br><span class="room">Lecture Room 3</span></td>
</tr>
<tr class="labs">
    <td>12:05</td>
    <td>&ndash;</td>
    <td>12:30</td>
    <td colspan="6">Best of Labs, 2025 (GutBrainIE, SimpleText, Touché)<br><span class="room">Lecture Room 3</span></td>
</tr>
<tr class="break">
    <td>12:30</td>
    <td>&ndash;</td>
    <td>14:00</td>
    <td colspan="6">Lunch Break &amp; Steering Committee Meeting</td>
</tr>
<tr class="labs">
    <td>14:00</td>
    <td>&ndash;</td>
    <td>15:30</td>
    <td>SimpleText 3/3</td>
    <td>LifeCLEF 3/3</td>
    <td>TalentCLEF 1/3</td>
    <td>BioASQ 1/4</td>
    <td>Touché 3/4</td>
    <td>ELOQUENT 1/3</td>
</tr>
<tr class="break">
    <td>15:30</td>
    <td>&ndash;</td>
    <td>16:00</td>
    <td colspan="6">Coffee Break &amp; Poster Session (HIPE, BioASQ, ELOQUENT)</td>
</tr>
<tr class="labs">
    <td>16:00</td>
    <td>&ndash;</td>
    <td>17:30</td>
    <td>ImageCLEF 1/5</td>
    <td>HIPE<br>1/1</td>
    <td>TalentCLEF 2/3</td>
    <td>BioASQ 2/4</td>
    <td>Touché 4/4</td>
    <td>ELOQUENT 2/3</td>
</tr>
</tbody>
</table>
{{< /rawhtml >}}

**Papers in Main Conference 2 (Bias, Fairness, Evidence)**
1. *Named Entity Masking for De-Biasing Text Classification: Experiments with German Datasets*<br>
  Daphne Kiok, Khushi Pitroda, Aenne Knierim and Thomas Mandl
2. *Do Physiological Signals Improve Both Generalization and Personal Perception of Sexism in Memes?*<br>
  Iván Arcos and Paolo Rosso
3. *A Controlled Study of Evidence Quality in LLM-Based Numerical Claim Verification*<br>
  Quy Thanh Le, Ismail Badache and Maamar El Amine Hamri


**Papers in Best of Labs, 2025 (GutBrainIE, SimpleText, Touché)**
1. *Trusting Gut Instincts: A Fine-Grained Error Analysis of Transformer-Based Structured Data Extraction from Gut-Brain Axis Publications*<br>
   Lasse Ryge Andersen, Mikkel Hagerup Dolmer, Marius Ihlen Gardshodn, Juan Manuel Rodriguez and Daniele Dell'Aglio
2. *Lexical Simplification for Scientific Texts: Revisiting SARI in the Era of Modern LLMs*<br>
   Nico Hofmann, Julian Dauenhauer, Nils Ole Dietzler, Idehen Daniel Idahor and Christin Katharina Kreutz
3. *DS@GT ARC at Touché: Large Language Models for Retrieval-Augmented Debate*<br>
  Anthony Miyaguchi and Conor Johnston

[//]: # (======================================================================================================================)
[//]: # (Wednesday)

## Wednesday, 23 September 2026
{{< rawhtml >}}
<table>
<colgroup>
    <col span="3" class="time">
    <col span="6" class="event">
</colgroup>
<thead>
    <tr>
        <th colspan="9">Wednesday, 23 September 2026</th>
    </tr>
</thead>
<tbody>
<tr class="main-conf">
    <td>09:00</td>
    <td>&ndash;</td>
    <td>10:00</td>
    <td colspan="6">Keynote: Suzan Verberne<br><span class="room">Lecture Room 3</span></td>
</tr>
<tr class="break">
    <td>10:00</td>
    <td>&ndash;</td>
    <td>10:30</td>
    <td colspan="6">Coffee Break &amp; Poster Session<br>(Best-of-Labs, PAN, CheckThat!, LongEval, eRisk, TalentCLEF, ImageCLEF)</td>
</tr>
<tr class="main-conf">
    <td>10:30</td>
    <td>&ndash;</td>
    <td>11:15</td>
    <td colspan="6">Main Conference 3 (Evaluation and Benchmarks)<br><span class="room">Lecture Room 3</span></td>
</tr>
<tr class="labs">
    <td>11:15</td>
    <td>&ndash;</td>
    <td>12:05</td>
    <td colspan="6">Lab Overviews 3 (PAN, LongEval, eRisk, CheckThat!, JOKER)<br><span class="room">Lecture Room 3</span></td>
</tr>
<tr class="labs">
    <td>12:05</td>
    <td>&ndash;</td>
    <td>12:30</td>
    <td colspan="6">Best of Labs, 2025 (CheckThat!, LongEval, JOKER)<br><span class="room">Lecture Room 3</span></td>
</tr>
<tr class="break">
    <td>12:30</td>
    <td>&ndash;</td>
    <td>14:00</td>
    <td colspan="6">Lunch Break</td>
</tr>
<tr class="labs">
    <td>14:00</td>
    <td>&ndash;</td>
    <td>15:30</td>
    <td>ImageCLEF<br>2/5</td>
    <td>PAN<br>1/4</td>
    <td>TalentCLEF<br>3/3</td>
    <td>BioASQ<br>3/4</td>
    <td>LongEval<br>1/2</td>
    <td>eRisk<br>1/2</td>
</tr>
<tr class="break">
    <td>15:30</td>
    <td>&ndash;</td>
    <td>16:00</td>
    <td colspan="6">Coffee Break &amp; Poster Session<br>(Best-of-Labs, PAN, CheckThat!, LongEval, eRisk, TalentCLEF, ImageCLEF)</td>
</tr>
<tr class="labs">
    <td>16:00</td>
    <td>&ndash;</td>
    <td>17:30</td>
    <td>ImageCLEF<br>3/5</td>
    <td>PAN<br>2/4</td>
    <td>CheckThat!<br>1/3</td>
    <td>BioASQ<br>4/4</td>
    <td>LongEval<br>2/2</td>
    <td>eRisk<br>2/2</td>
</tr>
<tr class="social">
    <td>18:15</td>
    <td></td>
    <td></td>
    <td colspan="6">Conference Dinner</td>
</tr>
</tbody>
</table>
{{< /rawhtml >}}

**Papers in Main Conference 3 (Evaluation and Benchmarks)**
1. *Evaluating Pre-Training Objectives for TITE*<br>
  Ferdinand Schlatt, Rayk Kretzschmar and Matthias Hagen
2. *KangarooBench: A Native German Benchmark for Evaluating the Modality Gap in Visual Mathematics*<br>
  Dennis Hug, René Wolf and Philippe Thomas
3. *ReTARgeting CLEF eHealth: From Historical TAR Benchmarks to Reusable Evaluation Resources*<br>
  Giorgio Maria Di Nunzio, Guglielmo Faggioli, Stefano Marchesin and Simone Merlo

**Papers in Best of Labs, 2025 (CheckThat!, LongEval, JOKER)**
1. *Scientific Claim–Source Retrieval Revisited: A Comparative Study of Style Transfer and Re-Ranking*<br>
   Tobias Schreieder, Harsh Khandelwal, Yu-Ling Zhong and Michael Färber
2. *DS@GT ARC at LongEval: Evaluating Temporal Performance in Web Search Systems and Topics with Two-Stage Retrieval*
   Anthony Miyaguchi, Imran Afrulbasha and Aleksandar Pramov
3. *Pun Intended: Multi-Agent Translation of Wordplay with Contrastive Learning and Phonetic-Semantic Embeddings*<br>
    Russell Taylor, Benajmin Herbert and Michael Sana

[//]: # (======================================================================================================================)
[//]: # (Thursday)

## Thursday, 24 September 2026
{{< rawhtml >}}
<table>
<colgroup>
    <col span="3" class="time">
    <col span="6" class="event">
</colgroup>
<thead>
    <tr>
        <th colspan="9">Thursday, 24 September 2026</th>
    </tr>
</thead>
<tbody>
<tr class="main-conf">
    <td>09:30</td>
    <td>&ndash;</td>
    <td>10:30</td>
    <td colspan="6">Main Conference 4 (Information Retrieval)<br><span class="room">Lecture Room 3</span></td>
</tr>
<tr class="break">
    <td>10:30</td>
    <td>&ndash;</td>
    <td>11:00</td>
    <td colspan="6">Coffee Break</td>
</tr>
<tr class="labs">
    <td>11:00</td>
    <td>&ndash;</td>
    <td>12:30</td>
    <td>ImageCLEF<br>4/5</td>
    <td>PAN<br>3/4</td>
    <td>CheckThat!<br>2/3</td>
    <td>JOKER<br>1/2</td>
    <td></td>
    <td></td>
</tr>
<tr class="break">
    <td>12:30</td>
    <td>&ndash;</td>
    <td>14:00</td>
    <td colspan="6">Lunch Break</td>
</tr>
<tr class="labs">
    <td>14:00</td>
    <td>&ndash;</td>
    <td>15:30</td>
    <td>ImageCLEF<br>5/5</td>
    <td>PAN &amp; ELOQUENT<br>4/4 &amp; 3/3</td>
    <td>CheckThat!<br>3/3</td>
    <td>JOKER<br>2/2</td>
    <td></td>
    <td></td>
</tr>
<tr class="break">
    <td>&ndash;</td>
    <td>16:00</td>
    <td colspan="6">Coffee Break</td>
</tr>
<tr class="main-conf">
    <td>16:00</td>
    <td>&ndash;</td>
    <td>17:30</td>
    <td colspan="6">Closing Session<br><span class="room">Lecture Room 3</span></td>
</tr>
</tbody>
</table>
{{< /rawhtml >}}

**Papers in Main Conference 4 (Information Retrieval)**
1. *Category-Conditioned Persona Retrieval for Multi-Session Dialogue: A Method and a Gold-Annotation-Compatible Retrieval Metric*<br>
  Konstantin Zaitsev
2. *Are Lexical Bindings Important for SPLADE?*<br>
  Ferdinand Schlatt, Sebastian Harm and Matthias Hagen
3. *Query Understanding with Large Language Models*<br>
  Eric Oliver Schmidt, Matthias Hagen and Maik Fröbe
4. *Scope Matters: A Hallucination-Aware Evaluation of Retrieval Strategies for Multilingual RAG*<br>
  Gadha Saji Menon, Swathi Jayakumar, Varalekshmy M Mohan, Sachin Kurup, Veena G and Vani Kanjirangat
